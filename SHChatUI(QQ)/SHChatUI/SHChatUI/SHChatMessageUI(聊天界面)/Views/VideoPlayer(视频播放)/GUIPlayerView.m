...
- (void)setup {
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playerStalled:) name:AVPlayerItemPlaybackStalledNotification object:nil];
  
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(airPlayAvailabilityChanged:) name:MPVolumeViewWirelessRoutesAvailableDidChangeNotification object:nil];
  
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(airPlayActivityChanged:) name:MPVolumeViewWirelessRouteActiveDidChangeNotification object:nil];
  ...
  //播放开始图片
  [playButton setImage:[UIImage imageNamed:@"video_play_normal"] forState:UIControlStateNormal];
  [playButton setImage:[UIImage imageNamed:@"video_play_selector"] forState:UIControlStateHighlighted];
  //播放暂停图片
  [playButton setImage:[UIImage imageNamed:@"video_stop_normal"] forState:UIControlStateSelected];
  ...
  //全屏
  [fullscreenButton setImage:[UIImage imageNamed:@"video_scan_normal"] forState:UIControlStateNormal];
  //非全屏
  [fullscreenButton setImage:[UIImage imageNamed:@"video_scan_normal"] forState:UIControlStateSelected];
  ...
  horizontalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|[P(40)][S(10)][C]-5-[I]-5-[R][F(40)][V(40)]|" options:0 metrics:nil views:@{@"P" : playButton, @"S" : spacerView,  @"C" : currentTimeLabel, @"I" : progressIndicator, @"R" : remainingTimeLabel,  @"V" : volumeView, @"F" : fullscreenButton}];
  ...
  horizontalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-5-[L]-5-|" options:0 metrics:nil views:@{@"L" : liveLabel}];
  
  [controllersView addConstraints:horizontalConstraints];
  
  for (UIView *view in [controllersView subviews]) {
    verticalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[V(40)]" options:NSLayoutFormatAlignAllCenterY metrics:nil views:@{@"V" : view}];
...
- (void)refreshProgressIndicator {
  ...
  if (duration == 0 || isnan(duration)) {
    ...
    progressIndicator.value * duration :
    // If seeking, reflects the position of the slider
    ...
    //整体时长
    NSDate *remainingTime = [NSDate dateWithTimeIntervalSince1970:duration];
...
- (void)showControllers {
  [UIView animateWithDuration:0.2f animations:^{
    ...
    if (controllersTimeoutPeriod > 0) {
      controllersTimer = [NSTimer scheduledTimerWithTimeInterval:controllersTimeoutPeriod target:self selector:@selector(hideControllers) userInfo:nil repeats:NO];
    }
  }];
}
...
- (void)play {
  ...
  progressTimer = [NSTimer scheduledTimerWithTimeInterval:0.1f target:self selector:@selector(refreshProgressIndicator) userInfo:nil repeats:YES];
}
