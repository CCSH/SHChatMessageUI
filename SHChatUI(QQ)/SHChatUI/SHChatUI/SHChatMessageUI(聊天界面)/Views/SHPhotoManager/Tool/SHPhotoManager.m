...
#import "SHPhotoManager.h"
#import "SHPhotoHeader.h"

@interface SHPhotoManager ()

@property (nonatomic ,strong) NSMutableArray<SHAblumInfo *> * ablumsList;

@end

@implementation SHPhotoManager


+ (instancetype)manager {
    
    static SHPhotoManager * manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[SHPhotoManager alloc]init];
    ...
    [obj enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([obj isKindOfClass:[PHAssetCollection class]]) {
            ...
            if (result.count) {
                ...
                //创建此相册的信息集
                SHAblumInfo * info = [SHAblumInfo infoFromResult:result collection:obj];
 ...
 * 获取资源对应的图片
 */
- (void)fetchImageInAsset:(PHAsset *)asset size:(CGSize)size isResize:(BOOL)isResize completeBlock:(void (^)(NSData *, NSString *))completeBlock {
    ...
    [[PHImageManager defaultManager] requestImageDataForAsset:asset options:option resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
        ...
        if (completeBlock) completeBlock(imageData, dataUTI?:@"jpg");
        //下方代码注掉经测试系统方法Data字节小 SIZE暂时保留
//        __block NSData *data = [NSData dataWithData:imageData];
//        if ([dataUTI containsString:@"gif"]) {
//            if (completeBlock) completeBlock(data, dataUTI);
//        }else{
//            //其他格式的图片
...
//                    data = [SHDataConversion imageToData:result CompressionNum:1];
//                    if (data) {
//                        if (completeBlock) completeBlock(data, dataUTI);
//                    }
//                }
...
- (void)fetchImagesWithAssetsArray:(NSMutableArray<PHAsset *> *)assetsArray isOriginal:(BOOL)isOriginal completeBlock:(void(^)(NSArray *images))completeBlock {
    ...
    for (int i = 0; i < assetsArray.count; i ++) {
        ...
        CGSize size = [self getImageCompressionWithAsset:asset isOriginal:isOriginal];
        
        [self fetchImageInAsset:asset size:size isResize:YES completeBlock:^(NSData *data, NSString *fileType) {
            
            UIImage *image = [UIImage imageWithData:data];
            
            //当图片读取到指定尺寸时
            if (image.size.width >= size.width * 0.9 || image.size.height >= size.height * 0.9) {
                
                [images addObject:@[data,fileType]];
