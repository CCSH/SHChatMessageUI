...
- (void)pauseAudio {
    if (_player) {
        ...
        if ([self.delegate respondsToSelector:@selector(didAudioPlayerPausePlay:)]) {
            [self.delegate didAudioPlayerPausePlay:_playingName];
        }
    }
}
#pragma mark - stop
...
- (void)stopAudio {
    ...
    if ([self.delegate respondsToSelector:@selector(didAudioPlayerStopPlay:error:)]) {
        [self.delegate didAudioPlayerStopPlay:_playingName error:nil];
    }
    _player = nil;
    _playingName = nil;
...
//            _playingName = audioPath;
            ...
            NSString *viceoName = [[audioPath.lastPathComponent componentsSeparatedByString:@"."] firstObject];
            
            //拼接WAV文件路径
            NSString *wavfile= [NSString stringWithFormat:@"%@/%@.wav",kSHAudioWAVPath,viceoName];
            //获取AMR文件路径
            NSString *amrFile= [NSString stringWithFormat:@"%@/%@.amr",kSHAudioAMRPath,viceoName];
...
- (void)startPlayAudioWithWavFile:(NSString *)wavFile{
    ...
    //播放开始回调
    if ([self.delegate respondsToSelector:@selector(didAudioPlayerBeginPlay:)]) {
        [self.delegate didAudioPlayerBeginPlay:_playingName];
    }
    ...
    if (!_player) {
        //播放错误
        if ([self.delegate respondsToSelector:@selector(didAudioPlayerStopPlay:error:)]) {
            [self.delegate didAudioPlayerStopPlay:_playingName error:[error description]];
        }
        //播放下一条
        [self playNext];
    }
...
- (NSString *)getAudioFileName{
    ...
    if (self.audioArr.count >= _playingIndex && _playingIndex != 0) {
        ...
        _playingName = message.messageId;
        
        switch (message.messageType) {
            case SHMessageBodyType_Voice://语音
            {
                if (message.voiceUrl.length) {
            ...
            {
                if (message.voiceEmail.length) {
                    return message.voiceEmail;
                }
            }
                break;
            default:
