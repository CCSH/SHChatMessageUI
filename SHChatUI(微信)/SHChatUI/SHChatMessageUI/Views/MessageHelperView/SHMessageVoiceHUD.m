...
- (void)showVoiceWithMessage:(NSString *)message{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!self.messageVoiceImage) {
            //图片
            self.messageVoiceImage = [[UIImageView alloc] initWithFrame:CGRectMake(10, 0, self.messageBgView.frame.size.width - 20, self.messageBgView.frame.size.height - 40)];
...
{
    if (![self.messageVoiceLabel.text isEqualToString:@"松开手指,取消发送"]) {
