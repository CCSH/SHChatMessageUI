...
//    line.frame = CGRectMake(button.frame.size.width - 1, button.frame.size.height/4, 1, button.frame.size.height/2);
...
- (void)reloadView{
    ...
    //工具栏上方添加分割线
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, self.height - 38, self.width , 0.6)];
...
- (void)emotionDidSelected:(NSNotification *)noti{
    ...
    for (NSString *obj in self.toolBarArr) {
        //如果有最近则进行添加
        if ([obj intValue] == SHEmoticonType_recent && button.emotion.type != SHEmoticonType_gif) {
            //添加到最近
