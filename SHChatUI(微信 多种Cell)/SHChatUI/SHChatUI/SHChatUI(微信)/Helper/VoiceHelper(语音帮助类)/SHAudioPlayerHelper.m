...
- (void)stopAudio {
    ...
    if ([self.delegate respondsToSelector:@selector(didAudioPlayerStopPlay:error:)]) {
        [self.delegate didAudioPlayerStopPlay:self.playMark error:nil];
    }
    _player = nil;
    self.playMark = nil;
}
...
//        } else {//不是上次播放的录音
            ...
            NSLog(@"语音路径：%@",viceoPath);
            
            //拼接WAV文件路径
            NSString *wavfile = [SHFileHelper getFilePathWithName:viceoPath type:SHMessageFileType_wav];
            //获取AMR文件路径
            NSString *amrFile = [SHFileHelper getFilePathWithName:viceoPath type:SHMessageFileType_amr];
...
- (void)startPlayAudioWithWavFile:(NSString *)wavFile{
    ...
    self.playMark = [SHFileHelper getNameWithUrl:wavFile];
    //开启距离感应
